...
class ThreeLayerConvNet(object):
    ...
    def __init__(self, input_dim=(3, 32, 32), num_filters=32, filter_size=7,
        ...
        self.params['W1'] = np.random.normal(loc=0, scale=weight_scale,
                                            size=(num_filters, input_dim[0],
                                                 filter_size, filter_size ))
        self.params['b1'] = np.zeros(num_filters)
        # there is no info about stride, but in class notes
        # they are encouraged to take stride as 1. So, do I.
        stride = 1
        # same story as above
        padding_size = (filter_size - 1) / 2
        _, H, W = input_dim
        # note that filter_size is both height and width.
        h_out = int((H - filter_size + 2 * padding_size) / stride + 1)
        w_out = int((W - filter_size + 2 * padding_size) / stride + 1)
        # it will decrease after 2x2 pooling
        h_out //= 2
        w_out //= 2
    ...
    def loss(self, X, y=None):
        ...
        reg_loss += self.reg * 0.5 * np.sum(W3*W3)
        loss = data_loss + reg_loss

        dout, grads['W3'], grads['b3'] = affine_backward(dx, cache2)
